...
% in line 38 (and potentially lines 39 and 40 if you organized your folders
% differently from me), and also line 68 if you decide not to put
% your .mat output files into a subfolder called 'mat'.  You'll also need
% to change line 42 to process both the super-res and normal-res data.
%
...
%                   corresponding netCDF files (.nc) exported from
%                   the NCDC Data Viewer.
%
...
%        cd(case_dir)
        ...
        [~, nvars, ngatts, ~] = netcdf.inq(ncid1); % unused: ndims, unlimdimid
        vname = cell(1, nvars);
        numatts = cell(1, nvars);
        type = cell(1, nvars);
        for jdx = 1:nvars
            [varname, xtype, ~, natts] = netcdf.inqVar(ncid1, jdx-1); % unused: dimids
            vname{jdx} = varname;
            numatts{jdx} = natts;
            type{jdx} = xtype;
        end
